...
4. PAGE COMPONENT MOUNTS
   └── LocalStorage hooks hydrate from storage
       ...
       ├── useWatchProgress()
       │   └── getSnapshot()
       │       └── localStorage.getItem("aniflix-watch-progress")
       ...
       ├── useSavedSeries()
       │   └── getSnapshot()
       │       └── localStorage.getItem("aniflix-saved-series")
       │           └── Returns: SavedSeries[]
       │
       └── usePlayerPreferences()
           └── getSnapshot()
               └── localStorage.getItem("aniflix-player-preferences")
...
| Key | Type | Contents |
|-----|------|----------|
| `aniflix-watch-progress` | `Record<string, WatchProgress>` | Episode progress keyed by `animeId:episodeNumber` |
| `aniflix-saved-series` | `SavedSeries[]` | Array of saved anime with metadata |
| `aniflix-player-preferences` | `PlayerPreferences` | Video player settings |
...
│           │                                                              │
│           └── localStorage.setItem("aniflix-watch-progress", JSON)       │
│               │                                                          │
